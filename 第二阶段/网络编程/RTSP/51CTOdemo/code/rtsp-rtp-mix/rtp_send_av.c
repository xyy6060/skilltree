...
void analyze_argv(char **argv, rtp_args_t *rtp_video_args, rtp_args_t *rtp_audio_args)
{
	...
	memcpy(rtp_video_args->media, argv[0], strlen(argv[0]));
	memcpy(rtp_video_args->ipaddr, argv[1], strlen(argv[1]));
	rtp_video_args->port = atoi(argv[2]);

	// analayze audio params
	memcpy(rtp_audio_args->media, argv[3], strlen(argv[3]));
	memcpy(rtp_audio_args->ipaddr, argv[1], strlen(argv[1]));
	rtp_audio_args->port = atoi(argv[4]);
